...
public:
    void setDebugConfig(bool value) {
        config_debug = value;
    }
protected:
    static void char_hex_out(const unsigned char &a)
    {
        const char HEX[] = "0123456789ABCDEF";
        int index = a & 0x0F;
        printf("%c%c",HEX[(a&0xF0)>>4],HEX[a&0x0F]);
    }
    void printHex(const unsigned char *str, uint64_t size)
    {
        if (str == NULL) return;
        int i = 0, j = 0;
        for (i = 0; i * 4 <= size; i++) {
            int start = i * 4;
            int end = std::min((i + 1) * 4, (int)size);
            if(start == end) break;
            for(j = start; j < end; j++) {
                char_hex_out(str[j]);
            }
            if(end < size) putchar('_');
        }
    }
    axi_resp do_read(uint64_t start_addr, uint64_t size, unsigned char* buffer) {
        ...
        if (it->first.first <= start_addr && end_addr <= it->first.second) {
            ...
            bool res = it->second->do_read(start_addr % dev_size, size, buffer);
            if(config_debug) {
                printHex(buffer, size);
                puts("");
            }
            return res ? RESP_OKEY : RESP_SLVERR;
        }
        else {
            if(config_debug) {
                puts("ERROR!!!");
    ...
    axi_resp do_write(uint64_t start_addr, uint64_t size, const unsigned char* buffer) {
        ...
        if (it->first.first <= start_addr && end_addr <= it->first.second) {
            uint64_t dev_size = it->first.second - it->first.first;
            bool res = it->second->do_write(start_addr % dev_size, size, buffer);
            if(config_debug) {
                printHex(buffer, size);
                puts("");
            }
